...
public class DList {
  ...
  public DList() {
    size = 0;
    head = newNode(Integer.MIN_VALUE, null, null);//will call dyanamic method(overridden newNode) even in constructor
    head.prev = head;
    head.next = head;
    System.out.println("D Constructor: (instanceof LockDListNode)" + (head instanceof LockDListNode));
  }
  ...
  public void insertFront(Object item) {
    DListNode frontNode = newNode(item, head, head.next);
    head.next = frontNode;
  ...
  public void insertBack(Object item) {
    DListNode backNode = newNode(item, head.prev, head);
    head.prev = backNode;
  ...
  public void insertAfter(Object item, DListNode node) {
    if (node != null) {
        DListNode nAfter = newNode(item, node, node.next);
        node.next = nAfter;
  ...
  public void insertBefore(Object item, DListNode node) {
    if (node != null) {
        DListNode nBefore = newNode(item, node.prev, node);
        node.prev = nBefore;
        nBefore.prev.next = nBefore;
        size++;
    }
  }
  ...
  public void remove(DListNode node) {
    if (node != null && node != head) {
        ...
        node.prev = null;//these two lines insure it is actually removed!
        node.next = null;//these two lines insure it is actually removed!
        size--;
    }
  }
  ...
  public static void main(String[] args) {
    ...
    //Describe one way can corrupt our DList:
