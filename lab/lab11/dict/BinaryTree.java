    ...
    if (node == null) {
      ...
      if (key.compareTo(node.entry.key()) == 0) {
        ...
        if (node.leftChild == null) {
          ...
          BinaryTreeNode findNode = findHelper((Comparable)key, node.leftChild);
          return findNode;
        }
      }
      else {
        if (node.rightChild == null) {
          ...
          BinaryTreeNode findNode = findHelper((Comparable)key, node.rightChild);
          return findNode;
        }
      }
    }
    ...
    if (node == null) {
      ...
      if (node.leftChild == null && node.rightChild == null) {
        ...
        if (node == root) {
          ...
          else if (node.rightChild != null) root = node.rightChild;
        }
        else if (node == node.parent.leftChild) {
          if (node.leftChild != null) node.parent.leftChild = node.leftChild;
          else if (node.rightChild != null) node.parent.leftChild = node.rightChild;
        }
        else if (node == node.parent.rightChild) {
          if (node.leftChild != null) node.parent.rightChild = node.leftChild;
          else if (node.rightChild != null) node.parent.rightChild = node.rightChild;
        }
      }
      else {
        BinaryTreeNode replaceNode = node.rightChild;
